...
candidate(X,Y) :-
    validIndices(X,Y), adjacent(X,Y,P,Q), visited(P,Q), \+ visited(X,Y).

adjacent(K,L,M,N) :-
    % In the same row
    validIndices(K,L), validIndices(M,N), L=N, Diff is K-M, abs(Diff) =:= 1. 
    
adjacent(K,L,M,N) :-
    % In the same column
    validIndices(K,L), validIndices(M,N), K=M, Diff is L-N, abs(Diff) =:= 1. 
...
noWumpus(P,Q) :- 
    % adjacent(P,Q,R,S), \+ stench(R,S). - doesn't work coz prolog assumes
